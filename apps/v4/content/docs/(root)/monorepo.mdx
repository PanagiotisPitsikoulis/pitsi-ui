---
title: Monorepo
description: Using pitsi/ui components and CLI in a monorepo.
---
...
For example, if you run `npx pitsi@latest add button`, the CLI will install the button component under `packages/ui` and update the import path for components in `apps/web`.

If you run `npx pitsi@latest add login-01`, the CLI will install the `button`, `label`, `input` and `card` components under `packages/ui` and the `login-form` component under `apps/web/components`.
