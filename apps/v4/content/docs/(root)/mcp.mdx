---
title: MCP Server
description: Use the pitsi MCP server to browse, search, and install components from registries.
---
...
Select your MCP client and follow the instructions to configure the pitsi MCP server. If you'd like to do it manually, see the [Configuration](#configuration) section.
...
<Tabs defaultValue="claude">
  ...
  <TabsContent value="claude" className="mt-4">
    **Run the following command** in your project:
       ```bash
       npx pitsi@latest mcp init --client claude
       ```
  ...
  <TabsContent value="cursor" className="mt-4">
    **Run the following command** in your project:
       ```bash
       npx pitsi@latest mcp init --client cursor
       ```

    Open **Cursor Settings** and **Enable the MCP server** for pitsi. Then try the following prompts:
       - Show me all available components in the pitsi registry
       - Add the button, dialog and card components to my project
       - Create a contact form using components from the pitsi registry
  ...
  <TabsContent value="vscode" className="mt-4">
    **Run the following command** in your project:
       ```bash
       npx pitsi@latest mcp init --client vscode
       ```

    Open `.vscode/mcp.json` and click **Start** next to the pitsi server. Then try the following prompts with GitHub Copilot:
       - Show me all available components in the pitsi registry
       - Add the button, dialog and card components to my project
       - Create a contact form using components from the pitsi registry
  ...
  <TabsContent value="codex" className="mt-4">
    <Callout className="mt-0">
      **Note:** The `pitsi` CLI cannot automatically update `~/.codex/config.toml`.
      You'll need to add the configuration manually for Codex.
    </Callout>

    **Run the following command** in your project:
       ```bash
       npx pitsi@latest mcp init --client codex
       ```
...
[Model Context Protocol (MCP)](https://modelcontextprotocol.io) is an open protocol that enables AI assistants to securely connect to external data sources and tools. With the pitsi MCP server, your AI assistant gains direct access to:
...
<Callout>
  **Note:** No configuration is needed to access the standard pitsi/ui
  registry.
</Callout>
...
- [Registry Documentation](/docs/registry) - Complete guide to pitsi registries
