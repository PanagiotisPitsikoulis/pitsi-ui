...
If you're starting a new registry project, you can use the [registry template](https://github.com/PanagiotisPitsikoulis-ui/registry-template) as a starting point. We have already configured it for you.
...
If you'd like to see an example of a registry, we have a [template project](https://github.com/PanagiotisPitsikoulis-ui/registry-template) for you to use as a starting point.
...
The `pitsi` CLI will automatically generate this file for you when you run the `build` command.
...
{
  "$schema": "https://ui.pitsi.com/schema/registry.json",
  "name": "acme",
...
### Install the pitsi CLI

```bash
npm install pitsi@latest
```
...
You can change the output directory by passing the `--output` option. See the [pitsi build command](/docs/cli#build) for more information.
...
To install a registry item using the `pitsi` CLI, use the `add` command followed by the URL of the registry item.

```bash
npx pitsi@latest add http://localhost:3000/r/hello-world.json
```
