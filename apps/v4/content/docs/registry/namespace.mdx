...
- `@pitsi/button` - UI component from the pitsi registry
...
```bash
npx pitsi@latest add @v0/dashboard
```

or multiple resources at once:

```bash
npx pitsi@latest add @acme/header @lib/auth-utils @ai/chatbot-rules
```
...
```bash
npx pitsi@latest add @acme/button
```
...
> **Note:** The `{name}` placeholder in the URL is automatically parsed and replaced with the resource name when you run `npx pitsi@latest add @namespace/resource-name`. For example, `@acme/button` becomes `https://registry.acme.com/resources/button.json`. See [URL Pattern System](#url-pattern-system) for more details.
...
```bash
npx pitsi@latest view @acme/button
```
...
When you run `npx pitsi@latest add @namespace/resource`, the CLI does the following:
...
```bash
npx pitsi@latest add @acme/auth @custom/login-form
```
...
```bash
npx pitsi@latest add @my-company/custom-button
```
...
```json title="dashboard.json" showLineNumbers
{
  "name": "dashboard",
  "registryDependencies": [
    "@pitsi/card", // 1. Resolved first
    "@vendor/chart", // 2. Resolved second
    "@custom/card" // 3. Resolved last (overrides @pitsi/card)
  ]
}
```
...
1. `@pitsi/card` - installs to `components/ui/card.tsx`
...
The pitsi CLI provides several commands for working with namespaced registries:
...
npx pitsi@latest add @acme/button @lib/utils @ai/prompt

# Install from URL directly
npx pitsi@latest add https://registry.example.com/button.json

# Install from local file
npx pitsi@latest add ./local-registry/button.json
```
...
npx pitsi@latest view @v0/dashboard @pitsi/card

# View from URL
npx pitsi@latest view https://registry.example.com/button.json
```
...
npx pitsi@latest search @v0

# Search with query
npx pitsi@latest search @acme --query "auth"

# Search multiple registries
npx pitsi@latest search @v0 @acme @lib

# Limit results
npx pitsi@latest search @v0 --limit 10 --offset 20
...
```bash
npx pitsi@latest add @non-existent/component
```
