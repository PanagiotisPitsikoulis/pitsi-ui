...
When you run `pitsi add` or `pitsi search`, the CLI will automatically check the registry index for the registry you are looking for and add it to your `components.json` file.

You can see the full list at [https://ui.pitsi.com/r/registries.json](https://ui.pitsi.com/r/registries.json).

## Adding a Registry

You can open an issue to add a registry to the index by filling out the [registry directory issue template](https://github.com/PanagiotisPitsikoulis/pitsi-ui/issues/new?template=registry_directory.yml).
