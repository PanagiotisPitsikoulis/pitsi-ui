...
// Default tools data
const pageTools1Defaults = {
  hero: {
    label: "Designer Tools",
    title: "Designer Tools",
    description:
      "Built to integrate seamlessly with your workflow. All the tools you need in one place.",
    primaryCta: { label: "Read more", href: "/tools-information" },
  ...
  tools: [
    ...
    {
      slug: "typography-composer",
      title: "Typography Composer",
      description: "Design type scales and pairings",
      icon: "typography",
    },
  ],
}
...
function ToolIcon({
  ...
  switch (icon) {
    ...
    case "shadow":
      return <ShadowIcon className={className} />
    case "easing":
      return <EasingIcon className={className} />
    case "spacing":
      return <SpacingIcon className={className} />
    case "typography":
      return <TypographyIcon className={className} />
    default:
      return <ThemeIcon className={className} />
  }
}
...
function ToolCard({
  ...
  tool: (typeof pageTools1Defaults.tools)[0]
  baseUrl?: string
}) {
  return (
    <Link
      href={`${baseUrl}/${tool.slug}`}
      className="group bg-background relative flex flex-col items-center rounded-2xl border px-6 pb-6 text-center shadow-sm"
    >
      <ArrowUpRight className="text-muted-foreground group-hover:text-primary absolute top-4 right-4 size-5 transition-colors" />
      <div className="text-primary py-8">
        <ToolIcon icon={tool.icon} className="size-32" />
      </div>
...
interface PageTools1Props {
  hero?: Partial<typeof pageTools1Defaults.hero>
  tools?: typeof pageTools1Defaults.tools
  baseUrl?: string
  className?: string
}

export function PageTools1({
  hero = {},
  tools = pageTools1Defaults.tools,
  baseUrl = "/tools",
  className,
}: PageTools1Props) {
  const heroContent = { ...pageTools1Defaults.hero, ...hero }

  return (
    <div className={cn("relative -mt-14 overflow-x-clip", className)}>
      <StripedBackground columnCount={6} />
      ...
      <div className="relative z-10 container px-6">
        ...
        <div className="mx-auto max-w-4xl">
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {tools.map((tool) => (
              <ToolCard key={tool.slug} tool={tool} baseUrl={baseUrl} />
            ))}
          </div>
        </div>
