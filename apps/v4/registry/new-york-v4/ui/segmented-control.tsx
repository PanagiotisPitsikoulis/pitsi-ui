...
>(({ children, className, floatingBgClassName, ...rest }, forwardedRef) => {
  ...
  return (
    <TabsPrimitive.List
      ...
      className={cn(
        "rounded-10 bg-background-elevation-0 border-border-elevation-0 relative isolate grid w-full auto-cols-fr grid-flow-col gap-1 border p-1",
        className
      )}
      {...rest}
    >
      ...
      <div
        className={cn(
          "bg-background-elevation-2 border-border-elevation-2 absolute inset-y-1 left-0 -z-10 rounded-md border transition-transform duration-300",
          {
            hidden: !mounted,
          },
          floatingBgClassName
  ...
  return (
    <TabsPrimitive.Trigger
      ...
      className={cn(
        // base
        "peer",
        "text-text-label-sm text-text-strength-300 relative z-10 h-7 rounded-md px-1 whitespace-nowrap outline-none",
        "flex items-center justify-center gap-1.5",
        "transition duration-300 ease-out",
        // focus
        "focus:outline-none",
        // active
        "data-[state=active]:text-text-strength-900",
        className
      )}
      {...rest}
    />
