...
// ----------------------------------------------------------------------------
// Step 3: Run start and capture simultaneously (changed items only)
// ----------------------------------------------------------------------------
async function runStartAndCapture(): Promise<void> {
  logStep(3, "Starting server and capturing changed items...")
    ...
    startProcess.stdout?.on("data", (data: Buffer) => {
      ...
      if (output.includes("Ready in") || output.includes("started server") || output.includes(":4000")) {
        if (!serverReady) {
          ...
          setTimeout(() => {
            // Start the capture process - uses default git-based detection (changed items only)
            log("  Starting registry capture (changed items only)...", "dim")
            captureProcess = spawn("bun", ["--bun", "./scripts/capture-registry.mts", "--verbose"], {
              cwd: V4_DIR,
              stdio: "inherit",
            })
...
async function main() {
  console.clear()
  log("=" .repeat(60), "cyan")
  log("  Registry Capture Workflow (Changed Items Only)", "cyan")
  log("=".repeat(60), "cyan")
  log("")
  log("This script will:", "dim")
  log("  1. Kill any running Next.js servers", "dim")
  log("  2. Build the application", "dim")
  log("  3. Start server and capture items changed in last commit", "dim")
  ...
  try {
    ...
    // Step 3: Start server and capture (changed items only)
