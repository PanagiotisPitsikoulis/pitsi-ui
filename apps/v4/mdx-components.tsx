...
import { DirectoryList } from "@/components/ui/directory-list"
import { CodeBlockCommand } from "@/components/documentation/code/code-block-command"
import { CodeCollapsibleWrapper } from "@/components/documentation/code/code-collapsible-wrapper"
import { CodeTabs } from "@/components/documentation/code/code-tabs"
import { CopyButton } from "@/components/documentation/code/copy-button"
import { ComponentPreview } from "@/components/documentation/components/component-preview"
...
export const mdxComponents = {
  ...
  th: ({ className, ...props }: React.ComponentProps<"th">) => (
    <th
      className={cn(
        "px-6 py-4 text-left font-bold [&[align=center]]:text-center [&[align=right]]:text-right",
        className
      )}
      {...props}
    />
  ...
  td: ({ className, ...props }: React.ComponentProps<"td">) => (
    <td
      className={cn(
        "px-6 py-4 text-left whitespace-nowrap [&[align=center]]:text-center [&[align=right]]:text-right",
        className
      )}
      {...props}
    />
  ...
  LinkedCard: ({ className, ...props }: React.ComponentProps<typeof Link>) => (
    <Link
      className={cn(
        "border-border bg-background hover:bg-background/80 flex w-full flex-col items-center rounded-xl border p-6 shadow-sm transition-colors sm:p-10",
        className
      )}
      {...props}
    />
