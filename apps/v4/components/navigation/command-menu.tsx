...
export function CommandMenu({
  ...
  const handlePageHighlight = React.useCallback(
    (isComponent: boolean, item: { url: string; name?: React.ReactNode }) => {
      if (isComponent) {
        ...
        setCopyPayload(
          `${packageManager} dlx pitsi@latest add ${componentName}`
        )
      } else {
  ...
  const handleBlockHighlight = React.useCallback(
    (block: { name: string; description: string; categories: string[] }) => {
      ...
      setCopyPayload(`${packageManager} dlx pitsi@latest add ${block.name}`)
    },
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild>
        <Button
          variant="secondary"
          className={cn(
            "border-border bg-sufrace relative h-8 w-auto rounded-full border px-3 shadow-sm sm:pr-12"
          )}
          onClick={() => setOpen(true)}
          {...props}
        >
          <RiSearchLine className="size-4" />
          <span className="ml-2 hidden pr-14 sm:inline">Search...</span>
          <div className="absolute top-1.5 right-1.5 hidden sm:flex">
            <KbdGroup className="gap-px">
              <Kbd className="border-border bg-sufrace aspect-square rounded-full border px-1.5 shadow-sm">
                ⌘
              </Kbd>
              <Kbd className="border-border bg-sufrace aspect-square rounded-full border px-1.5 shadow-sm">
                K
