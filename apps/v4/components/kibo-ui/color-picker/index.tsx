...
export const ColorPickerSelection = memo(
  ({ className, ...props }: ColorPickerSelectionProps) => {
    ...
    // saturation = x * 100, so x = saturation / 100
    // lightness formula: topLightness = x < 0.01 ? 100 : 50 + 50 * (1 - x)
    // lightness = topLightness * (1 - y), so y = 1 - lightness / topLightness
    const positionX = saturation / 100;
    const topLightness = positionX < 0.01 ? 100 : 50 + 50 * (1 - positionX);
    const positionY = topLightness > 0 ? 1 - lightness / topLightness : 0;
    ...
    const handlePointerMove = useCallback(
      (event: PointerEvent) => {
        ...
        setSaturation(x * 100);
        const newTopLightness = x < 0.01 ? 100 : 50 + 50 * (1 - x);
        const newLightness = newTopLightness * (1 - y);
