...
// Featured registry items for each category
...
function RegistryItemPreview({
  ...
  name: string
  type: "block" | "component" | "animation"
  aspectClass?: string
  className?: string
}) {
  ...
  return (
    <div
      ref={containerRef}
      className={`bg-background relative w-full overflow-hidden ${aspectClass} ${className}`}
    >
      {isInView && (
        <>
          {isLoading && (
...
export function ContentSection() {
  ...
  const scrollYProgress = useTransform(scrollY, (value) => {
    ...
    // Start when container top hits viewport top
    ...
    const progress = (value - scrollStart) / (scrollEnd - scrollStart)
    return Math.max(0, Math.min(1, progress))
  })

  // Tab opacities - sharper transitions to avoid overlap
  const tab0Opacity = useTransform(scrollYProgress, [0, 0.3, 0.33], [1, 1, 0])
  ...
  const tab0Scale = useTransform(scrollYProgress, [0, 0.3, 0.33], [1, 1, 0.8])
  const tab1Scale = useTransform(
    scrollYProgress,
    [0.3, 0.33, 0.63, 0.66],
    [1.1, 1, 1, 0.8]
  )
  const tab2Scale = useTransform(scrollYProgress, [0.63, 0.66, 1], [1.1, 1, 1])
  const tabScales = [tab0Scale, tab1Scale, tab2Scale]

  // Y translation - slide up/down with more distance
  const tab0Y = useTransform(scrollYProgress, [0, 0.3, 0.33], [0, 0, -60])
  const tab1Y = useTransform(
    scrollYProgress,
    [0.3, 0.33, 0.63, 0.66],
    [80, 0, 0, -60]
  )
  const tab2Y = useTransform(scrollYProgress, [0.63, 0.66, 1], [80, 0, 0])
  const tabYs = [tab0Y, tab1Y, tab2Y]

  // 3D rotations for cinematic depth
  const tab0RotateX = useTransform(scrollYProgress, [0, 0.3, 0.33], [0, 0, 25])
  const tab1RotateX = useTransform(
    scrollYProgress,
    [0.3, 0.33, 0.63, 0.66],
    [-25, 0, 0, 25]
  )
  const tab2RotateX = useTransform(
    scrollYProgress,
    [0.63, 0.66, 1],
    [-25, 0, 0]
  )
  const tabRotateXs = [tab0RotateX, tab1RotateX, tab2RotateX]

  // Z translation for depth
  const tab0Z = useTransform(scrollYProgress, [0, 0.3, 0.33], [0, 0, -200])
  const tab1Z = useTransform(
    scrollYProgress,
    [0.3, 0.33, 0.63, 0.66],
    [100, 0, 0, -200]
  )
  ...
  const getItemType = (tabId: string): "block" | "component" | "animation" => {
    if (tabId === "blocks") return "block"
    if (tabId === "animations") return "animation"
    return "component"
  }
