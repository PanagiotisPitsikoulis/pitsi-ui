...
function TestimonialCard({
  ...
  return (
    <div className={cn("rounded-3xl bg-card p-8", className)}>
...
export function TestimonialsServicePlants() {
  const testimonials = [
    ...
    {
      quote:
        "The care guide that came with my plants was incredibly helpful.",
      author: "James Liu",
      role: "Home Designer",
        ...
        <div className="grid gap-8 md:grid-cols-3">
          ...
          <div className="relative overflow-visible pb-8">
            ...
            <div className="absolute top-6 right-0 left-0 rotate-6">
              <TestimonialCard testimonial={stackedTestimonials[0]} className="bg-muted/50" />
            </div>
            {/* Middle stacked card */}
            <div className="absolute top-3 right-0 left-0 -rotate-4">
              <TestimonialCard testimonial={stackedTestimonials[1]} className="bg-muted/70" />
            </div>
            {/* Top card with overlay */}
            <div className="relative z-10">
              <TestimonialCard testimonial={stackedTestimonials[2]} className="bg-muted" />
              <div className="absolute inset-0 z-10 flex flex-col items-center justify-center gap-3 rounded-3xl bg-card/80 backdrop-blur-[2px]">
                <div className="flex -space-x-2">
                  {["07", "08", "09", "01", "02"].map((n) => (
                    <div key={n} className="relative h-10 w-10 overflow-hidden rounded-full border-2 border-card bg-muted">
                      <Image
                        draggable={false}
                        src={`/avatars/${n}.webp`}
                        alt=""
