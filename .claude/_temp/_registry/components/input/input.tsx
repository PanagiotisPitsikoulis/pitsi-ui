...
export const inputVariants = tv({
  slots: {
    root: [
      // base
      "group relative flex w-full overflow-hidden bg-background-elevation-0 text-text-strength-900",
      "transition duration-200 ease-out",
      "divide-x divide-border-elevation-0",
      // before
      "before:absolute before:inset-0 before:ring-1 before:ring-inset before:ring-border-elevation-0",
      "before:pointer-events-none before:rounded-[inherit]",
      "before:transition before:duration-200 before:ease-out",
      // hover
      "hover:shadow-none",
      // focus
      "has-[input:focus]:shadow-button-important-focus has-[input:focus]:before:ring-border-elevation-4",
      // disabled
      "has-[input:disabled]:shadow-none has-[input:disabled]:before:ring-transparent",
    ],
    wrapper: [
      // base
      "group/input-wrapper flex w-full cursor-text items-center bg-background-elevation-0",
      "transition duration-200 ease-out",
      // hover
      "hover:[&:not(&:has(input:focus))]:bg-background-elevation-1",
      // disabled
      "has-[input:disabled]:pointer-events-none has-[input:disabled]:bg-background-elevation-1",
    ],
    input: [
      // base
      "w-full bg-transparent bg-none text-paragraph-sm text-text-strength-900 outline-none",
      "transition duration-200 ease-out",
      // placeholder
      "placeholder:select-none placeholder:text-text-strength-400 placeholder:transition placeholder:duration-200 placeholder:ease-out",
      // hover placeholder
      "group-hover/input-wrapper:placeholder:text-text-strength-600",
      // focus
      "focus:outline-none",
      // focus placeholder
      "group-has-[input:focus]:placeholder:text-text-strength-600",
      // disabled
      "disabled:text-text-strength-100 disabled:placeholder:text-text-strength-100",
    ],
    icon: [
      // base
      ...
      "transition duration-200 ease-out",
      // placeholder state
      "group-has-[:placeholder-shown]:text-text-strength-400",
      // filled state
      "text-text-strength-600",
      // hover
      "group-has-[:placeholder-shown]:group-hover/input-wrapper:text-text-strength-600",
      // focus
      "group-has-[:placeholder-shown]:group-has-[input:focus]/input-wrapper:text-text-strength-600",
      // disabled
      "group-has-[input:disabled]/input-wrapper:text-text-strength-100",
    ],
    affix: [
      // base
      "shrink-0 bg-background-elevation-0 text-paragraph-sm text-text-strength-600",
      "flex items-center justify-center truncate",
      "transition duration-200 ease-out",
      // placeholder state
      "group-has-[:placeholder-shown]:text-text-strength-400",
      // focus state
      "group-has-[:placeholder-shown]:group-has-[input:focus]:text-text-strength-600",
    ],
    inlineAffix: [
      // base
      "text-paragraph-sm text-text-strength-600",
      // placeholder state
      "group-has-[:placeholder-shown]:text-text-strength-400",
      // focus state
      "group-has-[:placeholder-shown]:group-has-[input:focus]:text-text-strength-600",
