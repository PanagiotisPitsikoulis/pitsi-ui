...
function ColorThumb({ className, ...props }: AriaColorThumbProps) {
  return (
    <AriaColorThumb
      className={composeRenderProps(className, (className) =>
        cn("z-50 size-3 rounded-full ring-2 ring-static-white", className),
      )}
      {...props}
    />
  );
...
function ColorSwatch({ className, style, ...props }: AriaColorSwatchProps) {
  return (
    <AriaColorSwatch
      className={composeRenderProps(className, (className) =>
        cn(
          "size-4 rounded-full border-static-white group-data-[selected=true]/swatch-item:border-2 group-data-[selected=true]/swatch-item:ring-[1.5px]",
          className,
        ),
      )}
      style={({ defaultStyle }) => ({
