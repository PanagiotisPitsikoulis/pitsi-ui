  ...
  return (
    <AccordionPrimitive.Item
      ...
      className={cn(
        // base
        "group/accordion",
        "rounded-10 bg-background-elevation-0 p-3.5 ring-1 ring-inset ring-border-elevation-1",
...
>(({ children, className, ...rest }, forwardedRef) => {
  return (
    <AccordionPrimitive.Trigger
      ...
      className={cn(
        // base
        "w-[calc(100%+theme(space.7))] text-left text-label-sm text-text-strength-900",
...
function AccordionIcon<T extends React.ElementType>({
  ...
  return (
    <Component
      className={cn("size-5 text-text-strength-600", className)}
      {...rest}
    />
  );
}
...
>(({ children, className, ...rest }, forwardedRef) => {
  return (
    ...
    >
      <div
        className={cn(
          "pt-1.5 text-paragraph-sm text-text-strength-600",
          className,
        )}
      >
        {children}
...
export type AccordionProps = React.ComponentPropsWithoutRef<typeof AccordionPrimitive.Item>;
