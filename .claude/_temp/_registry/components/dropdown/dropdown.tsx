...
>(({ className, sideOffset = 8, ...rest }, forwardedRef) => (
  <DropdownMenuPrimitive.Portal>
    <DropdownMenuPrimitive.Content
      ...
      className={cn(
        "z-50 w-full overflow-hidden rounded-2xl bg-background-elevation-0 p-2 shadow-regular-md ring-1 ring-inset ring-border-elevation-1",
        "flex flex-col gap-1",
        // origin
...
>(({ className, inset, ...rest }, forwardedRef) => (
  <DropdownMenuPrimitive.Item
    ...
    className={cn(
      // base
      "group/item relative cursor-pointer select-none rounded-lg p-2 text-label-sm text-text-strength-900 outline-none",
...
>(({ className, ...rest }, forwardedRef) => (
  <DropdownMenuPrimitive.Label
    ref={forwardedRef}
    className={cn(
      "px-2 py-1 text-xs text-text-strength-600 font-medium",
      className,
    )}
    {...rest}
  />
...
>(({ className, inset, children, ...rest }, forwardedRef) => (
  <DropdownMenuPrimitive.SubTrigger
    ...
    className={cn(
      // base
      "group/item relative cursor-pointer select-none rounded-lg p-2 text-label-sm text-text-strength-900 outline-0",
      "flex items-center gap-2",
      "transition duration-200 ease-out",
      "border border-transparent",
      // hover
...
>(({ className, ...rest }, forwardedRef) => (
  <DropdownMenuPrimitive.SubContent
    ref={forwardedRef}
    className={cn(
      "z-50 w-max overflow-hidden rounded-2xl bg-background-elevation-0 p-2 shadow-regular-md ring-1 ring-inset ring-border-elevation-1",
      "flex flex-col gap-1",
      // animation
      "data-[state=open]:animate-in data-[state=open]:fade-in-0",
      "data-[state=closed]:animate-out data-[state=closed]:fade-out-0",
