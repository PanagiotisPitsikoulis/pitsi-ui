...
export function GalleryLines({
  ...
  return (
    <div
      className={cn(
        "h-auto bg-muted p-2 rounded-2xl",
    ...
    >
      <div
        className={cn(
          "flex h-full flex-row overflow-hidden rounded-xl bg-background",
          ...
          >
            {column.map((item, itemIndex) => (
              <a
                ...
                target="_blank"
                className={cn(
                  "group relative overflow-hidden rounded-lg opacity-75 transition-all hover:opacity-100 hover:scale-[1.02]",
                  classNames.link,
                )}
              >
...
function ExternalLinkLabel({
  ...
  return (
    <div
      className={cn(
        "absolute top-1/2 left-1/2 z-10 hidden -translate-x-1/2 -translate-y-1/2 transform flex-row items-center gap-[calc(1rem/16*7)] rounded-full border border-foreground bg-foreground/90 py-0.5 pr-2 pb-1 pl-3 text-center font-mono text-sm/6 font-medium text-background opacity-0 ring-1 ring-border transition-opacity group-hover:flex group-hover:opacity-100",
        className,
      )}
    >
      {children}
