...
>(({ children, className, floatingBgClassName, ...rest }, forwardedRef) => {
  ...
  return (
    <TabsPrimitive.List
      ...
      className={cn(
        "relative isolate grid w-full auto-cols-fr grid-flow-col gap-1 rounded-10 bg-background-elevation-0 border border-border-elevation-0 p-1",
        className,
      )}
      {...rest}
    >
      ...
      <div
        className={cn(
          "absolute inset-y-1 left-0 -z-10 rounded-md bg-background-elevation-2 border border-border-elevation-2 transition-transform duration-300",
          {
            hidden: !mounted,
          },
          floatingBgClassName,
  ...
  return (
    <TabsPrimitive.Trigger
      ...
      className={cn(
        // base
        "peer",
        "relative z-10 h-7 whitespace-nowrap rounded-md px-1 text-text-label-sm text-text-strength-300 outline-none",
