...
describe("validateRegistryConfig", () => {
  ...
  describe("built-in registries", () => {
    it("should not throw for @pitsi since it's now a built-in registry", () => {
      expect(() => {
        validateRegistryConfig("@pitsi", {
    ...
    it("should not throw for similar but different registry names", () => {
      expect(() => {
        validateRegistryConfig("@pitsi-ui", {
          url: "https://example.com/{name}",
        })
      }).not.toThrow()

      expect(() => {
        validateRegistryConfig("@mypitsi", {
          url: "https://example.com/{name}",
        })
      }).not.toThrow()
    })
