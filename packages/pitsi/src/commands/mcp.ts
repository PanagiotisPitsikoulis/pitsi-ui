...
const pitsi_MCP_VERSION = "latest"
...
const CLIENTS = [
  {
    ...
    config: {
      mcpServers: {
        pitsi: {
          command: "npx",
          args: [`pitsi@${pitsi_MCP_VERSION}`, "mcp"],
        },
      },
    },
  },
...
const DEPENDENCIES = [`pitsi@${pitsi_MCP_VERSION}`]
  ...
  .action(async (opts, command) => {
    try {
      ...
      if (options.client === "codex") {
        ...
        logger.break()
        logger.log("To configure the pitsi MCP server in Codex:")
        logger.break()
        logger.log(
