...
const pitsi_CLI_COMMAND = "pitsi@latest"

export async function npxpitsi(command: string) {
  const packageRunner = await getPackageRunner(process.cwd())
  return `${packageRunner} ${pitsi_CLI_COMMAND} ${command}`
}
...
export function formatSearchResultsWithPagination(
  ...
  const formattedItems = results.items.map((item) => {
    ...
    parts.push(
      `\n  Add command: \`${npxpitsi(`add ${item.addCommandArgument}`)}\``
    )
