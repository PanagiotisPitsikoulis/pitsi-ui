  ...
  return {
    tools: [
        ...
        inputSchema: zodToJsonSchema(
          z.object({
            registries: z
              .array(z.string())
              .describe(
                "Array of registry names to search (e.g., ['@pitsi', '@acme'])"
              ),
            limit: z
              .number()
              .optional()
        ...
        inputSchema: zodToJsonSchema(
          z.object({
            items: z
              .array(z.string())
              .describe(
                "Array of item names with registry prefix (e.g., ['@pitsi/button', '@pitsi/card'])"
              ),
          })
        ),
      },
      ...
      },
      {
        ...
        description:
          "Get the pitsi CLI add command for specific items in a registry. This is useful for adding one or more components to your project.",
        inputSchema: zodToJsonSchema(
          z.object({
            items: z
              .array(z.string())
              .describe(
                "Array of items to get the add command for prefixed with the registry name (e.g., ['@pitsi/button', '@pitsi/card'])"
              ),
          })
        ),
      },
          ...
          content: [
            {
              ...
              text: dedent`The following registries are configured in the current project:
                ...
                \`${await npxpitsi("view @name-of-registry")}\`
            ...
            content: [
              {
                ...
                Make sure the item names are correct and include the registry prefix (e.g., @pitsi/button).`,
              },
            ],
          }
        }
      ...
      case "get_add_command_for_items": {
        ...
        return {
          content: [
            {
              type: "text",
              text: await npxpitsi(`add ${args.items.join(" ")}`),
            },
          ],
        }
      }
